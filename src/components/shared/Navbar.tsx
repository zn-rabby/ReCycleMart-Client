...
export default function Navbar() {
  ...
  return (
    <header className="border-b w-full bg-white shadow-sm fixed top-0 z-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          {user ? (
            <>
              <Link href="/cart">
                <Button variant="ghost" className="rounded-full p-2 text-[#FF5E01] hover:bg-[#FF5E01]/10">
          ...
          <div className="p-4 flex flex-col gap-4">
            ...
            {user ? (
              <>
                <Link
                  href="/cart"
                  className="flex items-center gap-2 text-gray-700 hover:text-[#FF5E01]"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <Heart className="w-5 h-5" />
                  <span>Wishlist</span>
