...
//         const res = await createTracing(product.itemID);
...
import Image from "next/image";
import { useRouter } from "next/navigation";
import { toast } from "sonner";// <-- Import the server action

const ProductBuyDetails = ({ product }: { product: IProduct }) => {
  console.log("p p p",product._id)
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      const res = await createTransactions({itemID});  // Directly call server action
